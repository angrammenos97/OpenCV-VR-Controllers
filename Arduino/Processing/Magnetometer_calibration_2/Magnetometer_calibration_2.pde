...
final static int NUM_MAGN_SAMPLES = 20000;
...
void draw() {
  ...
  if (!synched) {
    ...
    textAlign(CENTER);
    fill(255);
    text("Connecting to Razor...", 0, 0, -200);
  ...
  //pushMatrix(); {
    ...
    if (magnetomIndex < NUM_MAGN_SAMPLES && serial.available() >= 36) {
      ...
      for (int i = magnetomIndex - 1 ; i >= 0 ; i--){
        // Draw new point
        pushMatrix();
        fill((magnetom[i][2] + 800) / 8, 255, 255);
        noStroke();
        translate(magnetom[i][0], magnetom[i][1], magnetom[i][2]);
        sphere(5);
        popMatrix();
      }
