...
void output_angles()
{
  if (output_format == OUTPUT__FORMAT_BINARY)
  {
    if (output_buttons)
    {
      ...
      readButtons(arduino_data);
      arduino_data.yaw = TO_DEG(yaw);
      arduino_data.pitch =  TO_DEG(pitch);
      arduino_data.roll =  -TO_DEG(roll);
      byte buff[sizeof(ArduinoData)] = { 0 };
      memcpy(buff , &arduino_data , sizeof(ArduinoData));
      Serial.write((const char *)buff , sizeof(ArduinoData));
    }
    else
    {
    float ypr[3];
  ...
  else if (output_format == OUTPUT__FORMAT_TEXT)
  {
    ...
    if (output_buttons)
    {
      ArduinoData arduino_data = { 0 };
      readButtons(arduino_data); 
      Serial.print(arduino_data.joyx);
      Serial.print(", ");
      Serial.print(arduino_data.joyy);
      Serial.print(", ");
      Serial.print(arduino_data.trigger);
      Serial.print(", ");
      Serial.println(arduino_data.buttons , BIN);
    }
  }
}
